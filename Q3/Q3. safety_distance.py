...
# CHANGED/ADDED FOR SERVICE: import custom service interface
from assignment1_rt_interfaces.srv import SetDistanceThreshold
...
class DistanceNode(Node):
    ...
    def __init__(self):
        ...
        # Replies: ok (bool)
        self.create_service(SetDistanceThreshold, '/set_distance_threshold', self.set_distance_threshold_cb)
    ...
    def set_distance_threshold_cb(self, request, response):
        ...
        if op == "increase":
            new_thr = self.min_allowed_distance + val
        elif op == "decrease":
            new_thr = self.min_allowed_distance - val
        else:
            response.ok = False
        ...
        # Prevent negative/too small thresholds
        if new_thr < 0.05:
            new_thr = 0.05

        self.min_allowed_distance = new_thr
        print(f"Threshold updated! min_allowed_distance = {self.min_allowed_distance:.3f}")
        response.ok = True
        return response
